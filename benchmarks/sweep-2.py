...
def drive(args):
    ...
    rows = []
    for i, (model, quant, eager, batch) in enumerate(configs, 1):
        label = "%s / %s / batch %d" % (
            quant or args.dtype, "eager" if eager else "cuda-graphs", batch)
        print("[%d/%d] %s" % (i, len(configs), label), flush=True)
        ...
        proc = subprocess.run(child_command(args, model, quant, eager, batch),
    ...
    if len(ok) > 1:
        ...
        print("cheapest : %.6f s  (%s, %s, batch %d)"
              % (best["seconds_per_inference"], best["quantization"],
                 "eager" if best["enforce_eager"] else "cuda-graphs", best["batch"]))
        print("dearest  : %.6f s  (%s, %s, batch %d)"
              % (worst["seconds_per_inference"], worst["quantization"],
                 "eager" if worst["enforce_eager"] else "cuda-graphs", worst["batch"]))
