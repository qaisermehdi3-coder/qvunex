...
def test_api_demo_shows_what_it_claims_to():
    """The demo is the only thing most people will ever run. If it stops
    surfacing the four findings the README promises, the README is lying."""
    import io
    import contextlib
    from qvunex import demo
    cwd = os.getcwd()
    with tempfile.TemporaryDirectory() as d:
        os.chdir(d)
        try:
            ...
            with contextlib.redirect_stdout(buf):
                demo.run_api_demo(tasks=8)
            a = analyse("./qvunex_api_demo.jsonl",
                        prices=prices.parse(demo.DEMO_PRICES))
        finally:
            os.chdir(cwd)

    kinds = {k["task"]: k for k in a["task_kinds"]}
    assert kinds["outbound email"]["n"] == 8
    assert kinds["outbound email"]["mean_usd"] > 0
    # a dearer model answered some calls without being asked for
    assert sum(k["fallbacks"] for k in a["task_kinds"]) > 0
    # thinking tokens came through the second provider's nested shape
    assert a["tokens"]["tokens_reasoning"] > 0
    # and that provider's inclusive prompt_tokens was normalised, so cached
    # tokens are not also sitting in the base-rate input count
    assert a["tokens"]["cache_read"] > 0
    out = render(a)
    assert "COST PER FINISHED TASK" in out
    assert "read none back" in out


def test_demo_does_not_double_when_run_twice():
    import io
    import contextlib
    from qvunex import demo
    cwd = os.getcwd()
        ...
        try:
            ...
            for _ in range(2):
                ...
                with contextlib.redirect_stdout(io.StringIO()):
                    demo.run_api_demo(tasks=4)
                totals.append(analyse("./qvunex_api_demo.jsonl",
                                      prices={})["total_calls"])
        finally:
            os.chdir(cwd)
